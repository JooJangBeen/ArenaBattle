...
public:
	//스탯이 업데이트될 때 호출할 함수.
	void UpdateState(const FABCharacterStat& BaseStat, const FABCharacterStat& ModifierStat);

private:
	//스탯 위젯이 가진 UI컨트롤중에 Base 스탯에 해당하는 텍스트 블록을 저장하는 맵.
	UPROPERTY()
	TMap<FName, class UTextBlock*> BaseLookup;

	//스탯 위젯이 가진 UI컨트롤중에 Modifier 스탯에 해당하는 텍스트 블록을 저장하는 맵.
	UPROPERTY()
