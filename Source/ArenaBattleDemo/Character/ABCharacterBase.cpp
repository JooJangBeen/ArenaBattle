...
AABCharacterBase::AABCharacterBase()
{
	...
	//매시의 콜리전은 NoCollision 설정(주로 랙돌에 사용됨).
	...
	static ConstructorHelpers::FObjectFinder<UABCharacterControlData> QuarterDataRef(
		TEXT("/Game/ArenaBattle/CharacterControl/ABC_QuarterDataAsset.ABC_QuarterDataAsset"));
	...
	static ConstructorHelpers::FObjectFinder<UABComboActionData> CombatActionDataRef(
		TEXT("/Game/ArenaBattle/ComboAction/ABA_ComboAction.ABA_ComboAction"));
	if (CombatActionDataRef.Object)
	{
		ComboActionData = CombatActionDataRef.Object;
	}
	...
	//컴포넌트 계층 설정 및 상대 위치 설정(머리위로 보일수 있게).
...
void AABCharacterBase::AttackHitCheck()
{
	...
	//트레이스에 사용할 구체의 반지름.
	const float AttakcRadius = Stat->GetAttackRadius();
...
void AABCharacterBase::ProcessComboCommand()
{
	...
	//콤보가 진행 중일때는 해당 프레임에 맞는 타이머를 활용.

	//콤보 타이머 핸들이 유효하지 않다면, 이미 콤보 발동이 걸렸거나 타이밍을 놓친 경우,
	//이 때는 콤보 처리가 필요하지 않음.
...
void AABCharacterBase::ComboActionBegin()
{
	...
	if (AnimInstance)
	{
		...
		//몽타주 재생이 시작 되면, 재생이 종료될 때 호출되는 델리게이트에 등록.
...
void AABCharacterBase::SetComboCheckTimer()
{
	...
	{
		//1.설정할 타이머 핸들.
		//2.타이머에 설정한 시간이 모드 지났을때 실행될 함수의 주인.
